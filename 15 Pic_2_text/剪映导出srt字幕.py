...
class KCJYSRT:
    ...
    def analyse_file(self, jy_json: dict) -> dict:
        ...
        subtitle_dic = sorted(subtitle_dic.items(),
                              key=lambda x: x[1]['start'], reverse=False)
        return subtitle_dic
    ...
    def make_str(self, subtitles: dict) -> str:
        ...
        for subtitle in subtitles:
            ...
            end = self.convert_timestr(
                subtitle[1]['start'] + subtitle[1]['duration'])
            _srt += start + ' --> ' + end
            _srt += '\n'
    ...
    def process_draft(self, draft_folder: str = ''):
        ...
        draft = os.path.join(draft_folder, "draft_content.json")

...
frame_2 = [[sg.T(' '), sg.T('每日耗电花费金额：'), sg.T('', key='output1', size=(25, 1), font=("微软雅黑", 10)), sg.Text('元')],
           [sg.Text('')],
           [sg.T(' '), sg.T('每月耗电花费金额：'), sg.T('', key='output2',
                                               size=(25, 1), font=("微软雅黑", 10)), sg.Text('元')],
           [sg.Text('')],
...
window = sg.Window('电费计算小助手，指导：张 飞', layout, font='微软雅黑',
                   icon=r"C:\Users\win\Desktop\ccc\Sites.ico")
...
while True:
   # 获取输入框和按钮元素的值
    ...
    if event == 'submit':
        ...
        if df:
            window.FindElement('output1').Update(df[0], text_color='black')
            window.FindElement('output2').Update(df[1], text_color='black')
            window.FindElement('output3').Update(df[2], text_color='black')
        else:
